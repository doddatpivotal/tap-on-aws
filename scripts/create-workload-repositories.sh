...
aws ecr create-repository --repository-name tanzu-application-platform/$1-$DEV_NAMESPACE --region $AWS_REGION --output json

# Create repository for workload K8s YAML generated by the supply chain (when using registry-ops)
aws ecr create-repository --repository-name tanzu-application-platform/$1-$DEV_NAMESPACE-bundle --region $AWS_REGION --output json
